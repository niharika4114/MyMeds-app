...
def book_appointment():
    ...
    if request.method == "POST":
        ...
        hour_24 = hour + \
            12 if ampm == "PM" and hour != 12 else (
                0 if hour == 12 and ampm == "AM" else hour)
        ...
        return redirect(url_for("patient_dashboard"))
    doctors = [u for u in users_table.scan().get(
        "Items", []) if u.get("user_type") == "doctor"]
        ...
        if sns and SNS_TOPIC_ARN:
            try:
                sns.publish(TopicArn=SNS_TOPIC_ARN,
                            Subject="MedTrack+ Reminder", Message=f"{message} at {time}")
            except Exception as e:
                flash(f"SNS error: {e}", "warning")
        send_email_reminder(
            session["user_email"], "MedTrack+ Reminder", f"Reminder: {message} at {time}")
...
def doctor_dashboard():
    ...
    return render_template("doctor_dashboard.html",
                           appointments=[
                               a for a in apps if a["doctor_email"] == email],
                           prescriptions=[
                               p for p in pres if p["doctor_email"] == email],
